...
def evaluate_model(model: nn.Module, 
                  ...
                  device: str,
                  model_type: str) -> Tuple[Dict[str, float], np.ndarray, np.ndarray]:
    """
    ...
    with torch.no_grad():
        for batch_x, batch_y in test_loader:
            ...
            all_preds.extend(outputs_np.flatten())
            all_actuals.extend(batch_y_np.flatten())
...
def main():
    ...
    try:
        ...
        train_loader, test_loader, _ = data_preprocessing.get_data_loaders(
            file_path="Scats_Data_October_2006.xlsx",
            batch_size=32,
            seq_len=24
        )
    except Exception as e:
        ...
        print("Ensure Scats_Data_October_2006.xlsx is in the correct directory")
        return
    
    if not train_loader or not test_loader:
        print("Error: Data loaders are empty")
        return
    ...
    for name, model in models.items():
        ...
        metrics, preds, acts = evaluate_model(
            model, test_loader, criterion, device, name.lower()
        )
...
#                   device: str,
#                   model_type: str,
#                   scaler) -> Tuple[Dict[str, float], np.ndarray, np.ndarray]:
#     """
#     Unified evaluation function for all model types (SAE, GRU, LSTM)
...
#                 outputs_np = scaler.inverse_transform(outputs_np.reshape(-1, 1)).flatten()
#                 batch_y_np = scaler.inverse_transform(batch_y_np.reshape(-1, 1)).flatten()
...
#         if not hasattr(data_preprocessing, 'get_data_loaders'):
#             raise AttributeError("get_data_loaders not found in data_preprocessing")
#         train_loader, test_loader, scaler = data_preprocessing.get_data_loaders(
#             file_path="Scats_Data_October_2006.xlsx",
#             batch_size=32,
#             seq_len=24
#         )
#         print(f"Scaler range: min={scaler.data_min_[0]:.2f}, max={scaler.data_max_[0]:.2f}")
#     except Exception as e:
#         print(f"Error loading data_preprocessing.get_data_loaders: {str(e)}")
#         print("Please verify data_preprocessing.py contains a working get_data_loaders function")
#         print("Ensure Scats_Data_October_2006.xlsx is in C:\\Users\\User\\Desktop\\Intro_To_AI\\assignment_final")
#         return
...
#         print(f"\nEvaluating {name}...")
#         metrics, preds, acts = evaluate_model(
#             model, test_loader, criterion, device, name.lower(), scaler
#         )
