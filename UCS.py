...
def ucs_search(nodes, edges, origin, destinations):
    visited = set()
    queue = []
    ...
    while queue:
        ...
        if current not in visited:
            visited.add(current)
            for neighbor, edge_cost in sorted(edges.get(current, [])):
                if neighbor not in visited:
                    new_path = list(path)
                    new_path.append(neighbor)
                    heapq.heappush(queue, (cost_so_far + edge_cost, new_path))
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python search.py <filename> ucs")
        sys.exit(1)
    ...
    if method != "ucs":
        print("Only 'ucs' method is supported in this script.")
        sys.exit(1)

    nodes, edges, origin, destinations = parse_input_file(filename)
    goal, num_nodes, path = ucs_search(nodes, edges, origin, destinations)
